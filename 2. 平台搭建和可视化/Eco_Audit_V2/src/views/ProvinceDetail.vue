<!-- <template>
  ...
  li:hover {
   ...
   <!-- <template>
    <div>
      ...
      <div class="buttons">
        <button @click="showStats">数据统计</button>
        <button @click="modifyData">数据修改</button>
      </div>
  
      
      <div v-if="selectedRiver">
  ...
  export default {
    ...
    data() {
      return {
        rivers: ['河流A', '河流B', '河流C'], // 省份下的河流列表
        selectedRiver: null, // 用户点击选择的河流
    ...
    methods: {
      ...
      selectRiver(river) {
        ...
        this.fetchData(); // 每次点击河流重新获取数据并绘制图表
      },
  
      // 预留“数据统计”和“数据修改”功能的按钮操作
      showStats() {
        // 这里是预留的数据统计逻辑
        alert('数据统计功能尚未实现');
      },
      ...
      // 从后端获取CSV数据并解析
      async fetchData() {
        // 请求CSV文件（根据河流不同请求不同的文件）
        const response = await axios.get(`/public/${this.selectedRiver}202101-202112.csv`);
  ...
  <div class="main-container">
    ...
    <div v-if="showSection === 'riverDetails'">
      ...
      <div v-if="selectedRiver && showChart" id="chart" style="width: 100%; height: 400px;"></div>
    </div>
  </div>
</template>  
...
//          this.showChart = true; // 显示图表容器
//          this.$nextTick(() => { // 确保DOM更新后再初始化图表
//            this.initChart();
//          });
//        },
//      });
